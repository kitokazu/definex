...
import { ArrowUpRight, Lock, Activity } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <FadeIn className="mb-24 text-center md:text-left">
          <div className="flex flex-col md:flex-row md:items-baseline space-y-2 md:space-y-0 md:space-x-6">
            <h2 className="text-5xl md:text-6xl font-black text-white italic tracking-tighter drop-shadow-lg">OUR SERVICES</h2>
                 ...
                 <div className="grid grid-cols-1 lg:grid-cols-2">
                    <div className="p-8 md:p-12 lg:p-16 flex flex-col justify-center order-2 lg:order-1 relative z-10">
                      ...
                      <h3 className="text-3xl md:text-4xl font-bold text-white mb-6">
                        スポーツ英会話 <br />
                        <span className="text-brand italic text-4xl md:text-5xl block mt-1 tracking-tight text-shadow-glow">DEFINE ACADEMY</span>
                      </h3>
                      <p className="text-slate-400 text-lg leading-relaxed mb-8 font-medium">
                        スポーツをテーマにしたオンライン英会話レッスン。<br />
                      ...
                      <div className="flex flex-wrap gap-3 mb-10">
                        {['Online', '1-on-1', 'Sports English', 'Custom Curriculum'].map(tag => (
                          <span key={tag} className="px-3 py-1 border-l-2 border-brand text-slate-300 text-sm font-bold uppercase tracking-wide bg-slate-800/50">
                            {tag}
                          </span>
                        ))}
                      </div>
